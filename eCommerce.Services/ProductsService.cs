    ...
    public class ProductsService
    {
        ...
        public List<Product> SearchProducts(List<int> categoryIDs, string searchTerm, decimal? from, decimal? to, string sortby, int? pageNo, int recordSize, bool activeOnly, out int count, int? stockCheckCount = null)
        {
            ...
            if (categoryIDs != null && categoryIDs.Count > 0)
            {
                ...
                {
                    var cat = GetProductByCategoriesID(categoryIDs);
                    products = products.Where(x => cat.Contains(x.CategoryID));
                }
                else
                {
        ...
        public List<int> GetProductByCategoriesID(List<int> categoryIDs)
        {
            var context = DataContextHelper.GetNewContext();
            if (categoryIDs.Count>1)
            {
            ...
            var cat = context.Categories.Where(x => categoryIDs.Contains((int)x.ParentCategoryID));

            return cat.Select(x => x.ID).ToList();
        }
        ...
        public decimal GetMaxProductPrice()
        {
            ...
            var products = context.ProductDtls.Where(x => !x.IsDeleted && x.IsActive);

            return products.Count() > 0 ? products.Max(x => x.Amount) : 0;
