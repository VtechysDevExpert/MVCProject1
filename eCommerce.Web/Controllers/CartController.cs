    ...
    public class CartController : PublicBaseController
    {
        ...
        public ActionResult Cart(bool isPopup = false)
        {
            ...
            if (SessionHelper.CartItems != null && SessionHelper.CartItems.Count > 0)
            {
                ...
                model.ProductDetails_vm = ProductDetailsService.Instance.GetProductDetails();
            ...
            {
                var promo = PromosService.Instance.GetPromoCode();
        ...
        public ActionResult UpdateCart(FormCollection formCollection)
        {
            ...
            if (SessionHelper.CartItems != null)
            {
                var productIDs = cartItemsUpdate.CartItems != null &&
                                   ...
                                   cartItemsUpdate.CartItems.Select(x => x.ID ).ToList() : new List<int>();

                if (productIDs.Count > 0)
                {
                    model.ProductDetails_vm = ProductDetailsService.Instance.GetProductsByIDs(productIDs);

                    model.Products = ProductsService.Instance.GetProductsByIDs(model.ProductDetails_vm.Select(X=>X.ProductID).ToList());
                }

                SessionHelper.CartItems.Clear();

                if (model.ProductDetails_vm != null && model.ProductDetails_vm.Count > 0)
                {
                    foreach (var product in model.ProductDetails_vm)
                    {
                        ...
                        {ID=product.ID, 
                            ...
                            Quantity = cartItemsUpdate.CartItems.FirstOrDefault(x => x.ID == product.ID).Quantity
                        });
            ...
            foreach (string key in formCollection)
            {
                if (key.Contains("product_"))
                {
                    ...
                    if (!string.IsNullOrEmpty(value))
                    {
                        ...
                        {
                            ID = int.TryParse(key.GetSubstringText("_", ""), out int productID) ? productID : 0,
        ...
        public JsonResult AddItemToCart(int itemID, int? ID, int quantity = 1)
        {
            ...
            if (ID != null )
            {
              ...
              //  var product = ProductsService.Instance.GetProductByID(itemID);
                var prodcutDetails = ProductDetailsService.Instance.GetProductDetailsByID(ID);
                if (prodcutDetails != null)
                {

                    if (prodcutDetails.StockQuantity > 0)
                    {
                        //var message = string.Empty;

                        var itemInCart = SessionHelper.CartItems.FirstOrDefault(x => x.ItemID == prodcutDetails.ProductID && x.Weight==prodcutDetails.Weight);
                        ...
                        {
                            //var productPrice = prodcutDetails.Discount.HasValue && prodcutDetails.Discount.Value > 0 ? prodcutDetails.Discount.Value : prodcutDetails.Amount;
                            var productPrice =prodcutDetails.Amount;

                            //add the product to cart.
                            SessionHelper.CartItems.Add(new CartItem() {ID=prodcutDetails.ID, ItemID = prodcutDetails.ProductID, Price = productPrice, Quantity = quantity,Weight=prodcutDetails.Weight });
            ...
            {
                json.Data = new { Success = false, Message = "Please select weight/price." };
            }
            return json;
        }
        ...
        public JsonResult GetAddress(int id_address)
        {
            
            var add=UserProfileService.Instance.GetAddress(id_address);
            JsonResult json = new JsonResult
            {
                Data = new { Success = true, address = add }
            };
            return json;
        }
        ...
        public ActionResult Checkout()
        {
            ...
            if (SessionHelper.CartItems != null && SessionHelper.CartItems.Count > 0)
            {
               ...
               model.User_Addresses = UserProfileService.Instance.GetAddressList(User.Identity.GetUserId());
               //instead of loading whole state we can pass stateid as parameter and fetch only states that user address have.
                model.states = UserProfileService.Instance.GetStateList();
                if (model.ProductIDs.Count > 0)
                {
                    model.ProductDetails_vm = ProductDetailsService.Instance.GetProductsByIDs(model.ProductIDs);
