...
namespace eCommerce.Web.Controllers
{
    public class HomeController : PublicBaseController
    {
        ...
        public ActionResult HomeBlog()
        {
            ...
            {
                BlogConfigurations = ConfigurationsService.Instance.GetConfigurationsByType((int)ConfigurationTypes.Blog)
            };

            return PartialView("_Blog", model);
        }
        public ActionResult HomeReview(int recordSize = 3)
        {
            CustomerReviewViewModel model = new CustomerReviewViewModel
            {
                CustomerReviews = CustomerReviewService.Instance.GetReviews(recordSize: recordSize)
            };

            return PartialView("CustomerReviews", model);
        }
        ...
        public ActionResult Search(string category, string q, decimal? from, decimal? to, string sortby, int? pageNo, int? recordSize)
        {
            ...
            {
                var selectedCategory = CategoriesService.Instance.GetCategoryByName(q);
